...
DateTime.fromHuman = function fromHuman(string, options = {}) {
	return parser.attempt(string, options.locale || defaultLocale);
};
...
// 	try {
// 		parseplus.regexes.MONTHNAME =
// 			moment.months().join('|') + '|' + moment.monthsShort().join('|');
// 		parseplus.regexes.DAYNAME =
// 			moment.weekdays().join('|') + '|' + moment.weekdaysShort().join('|');
...
// 		parseplus.regexes.ORDINAL = ordinalParse.source.replace(
// 			/.*\(([^)]+)\).*/,
// 			'$1'
// 		);
...
// 		//              $1            $2        $3           $4           $5                   $6             $7
// 		template:
// 			'^(?:(.+?)(?: |T))?(_H24_)\\:(_MIN_)(?:\\:(_SEC_)(?:\\.(_MS_))?)? ?(?:GMT)? ?(_TIMEZONE_)? ?(_TZABBR_)?$',
...
// 		//                                    $1                   $2                        $3
// 		template:
// 			'^(?:(?:_DAYNAME_)\\.?,? )?(_DAY_)(?:_ORDINAL_)?([ -])(_MONTHNAME_)\\.?\\2(_YEAR_)$',
// 		format: 'DD * MMM YYYY',
// 	})
...
// 		//                                    $1                       $2
// 		template:
// 			'^(?:(?:_DAYNAME_)\\.?,? )?(_DAY_)(?:_ORDINAL_)?[ -](_MONTHNAME_)\\.?$',
// 		handler: function (match) {
// 			return moment(
// 				[match[1], match[2], new Date().getFullYear()].join(' '),
// 				'DD MMM YYYY'
...
// 		//                                    $1                $2                      $3
// 		template:
// 			'^(?:(?:_DAYNAME_)\\.?,? )?(_MONTHNAME_)\\.? (_DAY_)(?:_ORDINAL_)?,? (_YEAR_)$',
// 		replacer: '$1 $2 $3',
// 		format: 'MMM DD YYYY',
// 	})
...
// 		//                                    $1                $2
// 		template:
// 			'^(?:(?:_DAYNAME_)\\.?,? )?(_MONTHNAME_)\\.? (_DAY_)(?:_ORDINAL_)?$',
// 		handler: function (match) {
// 			return moment(
// 				[match[1], match[2], new Date().getFullYear()].join(' '),
// 				'MMM DD YYYY'
